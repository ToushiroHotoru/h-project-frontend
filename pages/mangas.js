...
import MangaTile from "../components/Manga/MangaTile";
import MangaTileSkeleton from "../components/Manga/MangaTileSkeleton";
      ...
      <div className={catalog.catalog}>
        <div className="container">
          <div className={`${catalog.wrap} ${catalog.grid}`}>
            {[...Array(24)].map((_, i) => {
              return <MangaTileSkeleton key={i + 1} />;
            })}
          </div>
        </div>
      </div>
  ...
  return (
    <div className={catalog.catalog}>
      <div className="container">
        <div className={`${catalog.wrap} ${catalog.grid}`}>
          {data.map((item, i) => {
            return <MangaTile props={item} key={i + 1} />;
          })}
        </div>
      </div>
    </div>
